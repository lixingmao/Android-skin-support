...
public class SkinCompatViewInflater {
    ...
    private static Context themifyContext(Context context, AttributeSet attrs,
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SkinCompatView, 0, 0);
        int themeId = 0;
        if (useAndroidTheme) {
            // First try reading android:theme if enabled
            themeId = a.getResourceId(R.styleable.SkinCompatView_android_theme, 0);
        }
        if (useAppTheme && themeId == 0) {
            // ...if that didn't work, try reading app:theme (for legacy reasons) if enabled
            themeId = a.getResourceId(R.styleable.SkinCompatView_theme, 0);
